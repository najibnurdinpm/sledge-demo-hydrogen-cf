    ...
    >
      <div>
        <Trigger
          onAfterAddWishlist={() => {
            gtag.event({
              action: 'click_wishlist_trigger_app',
              category: 'apps',
              label: `click_wishlist_trigger_app`,
              value: 'add',
            });
          }}
              ...
              {optionValues[0] !== 'Default Title' && (
                ...
                ${
                  selectedOption === item
                    ...
                    const getLocalStorageSettings: string =
                      localStorage.getItem('sledge-instant-search-setting') ||
                      '{}';
                    const instantSearchSetting: any = JSON.parse(
                      getLocalStorageSettings,
                    );
                    const colorDataSettings =
                      instantSearchSetting?.colors || [];

                    const getColorSwatch =
                      colorDataSettings?.filter(
                        ({name}: any) => name === item,
                      )[0] || {};
                    ...
                    return (
                      <button
                        ...
                        style={{
                          background:
                            optionName === 'Color' ? colorSwatch : null,
