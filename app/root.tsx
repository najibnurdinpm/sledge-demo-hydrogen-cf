...
export async function loader({request, context}: LoaderArgs) {
  ...
  const {customer, shop} = await storefront.query(CUSTOMER_QUERY, {
    variables: {
      ...
      <body className={`bg-dark text-white antialiased ${productClass}`}>
        ...
        >
          <div
            onClick={() => {
              gtag.event({
                action: 'click_sticky_sidebar_widget_app',
                category: 'apps',
                label: `click_sticky_sidebar_widget_app`,
                value: pathname,
              });
            }}
          >
...
const CART_QUERY = `#graphql
  ...
  fragment MoneyRootFragment on MoneyV2 {
    currencyCode
    amount
  }
...
const CUSTOMER_QUERY = `#graphql
  ...
  fragment CustomerRootDetails on Customer {
    id
    firstName
    lastName
    phone
