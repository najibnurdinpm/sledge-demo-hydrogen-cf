...
import {defer, redirect, type LoaderArgs} from '@remix-run/cloudflare';
import {
  useLoaderData,
  Await,
  useLocation,
...
// Sledge Package
import {Rating, Snippet} from '@sledge-app/react-product-review';
...
export default function MainProduct() {
  ...
  const tracRecentlyViewed: any = useInView(trackOption);
  ...
  useEffect(() => {
    if (trackReview.entry?.isVisible && !isReviewTracked) {
      ...
      gtag.event({
        action: 'view_product_review_app',
        category: 'apps',
        label: `view_product_review_app`,
        value: product.title,
      });
    }
    if (tracRecentlyViewed.entry?.isVisible && !isRecentlyViewedTracked) {
      setIsRecentlyViewedTracked(true);
      gtag.event({
        action: 'view_recently_viewed_app',
        category: 'apps',
        label: `view_recently_viewed_app`,
        value: product.title,
      });
    }
  }, [trackReview, tracRecentlyViewed]);
  ...
  return (
    <>
      ...
      <div className="max-w-[1310px] mx-auto px-5 lg:px-20 xl:px-0 mb-[64px]">
        <div
          ref={tracRecentlyViewed.ref}
          className="sledge_recently-viewed_product-page"
        >
          <SledgeCustom.SledgeRecentlyViewed
    ...
    <>
      ...
      <div className="mt-[24px]">
        <div className="max-w-[480px]">
          <Snippet productId={product.id} />
          <h2 className="sr-only">Description</h2>
          <p
            className="font-inter font-medium text-[14px] text-dark4 lg:text-[16px] leading-[25.6px] tracking-[-0.32px] mt-[16px]"
            ...
            <>
              ...
              <div className="flex flex-wrap gap-[12px] mt-[32px] max-w-[350px]">
                ...
                <ButtonDetail
                  onAfterAddWishlist={() => {
                    gtag.event({
                      action: 'click_wishlist_button_detail_app',
                      category: 'apps',
                      label: `click_wishlist_button_detail_app`,
                      value: 'add',
                    });
                  }}
